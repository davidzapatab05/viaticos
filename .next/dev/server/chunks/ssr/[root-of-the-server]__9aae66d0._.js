...
function InstallPWA() {
    ...
    const isStandalone = ("TURBOPACK compile-time value", "undefined") !== 'undefined' && (window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone || document.referrer.includes('android-app://'));
    const onClick = (evt)=>{
        evt.preventDefault();
        if (!promptInstall) {
            return;
